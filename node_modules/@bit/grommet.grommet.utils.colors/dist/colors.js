...
var parseHexToRGB = function parseHexToRGB(color) {
  ...
  ? color.match(/[A-Za-z0-9]{1}/g).map(function (v) {
    return parseInt("".concat(v).concat(v), 16);
...
var getRGBArray = function getRGBArray(color) {
  if (hexExp.test(color)) {
    var _parseHexToRGB = parseHexToRGB(color),
        _parseHexToRGB2 = _slicedToArray(_parseHexToRGB, 4),
        red = _parseHexToRGB2[0],
        green = _parseHexToRGB2[1],
        blue = _parseHexToRGB2[2],
        alpha = _parseHexToRGB2[3];
  ...
  if (match) {
    var _match$splice$map = match.splice(1).map(function (v) {
      return parseInt(v, 10);
        ...
        h = _match$splice$map2[0],
        s = _match$splice$map2[1],
        l = _match$splice$map2[2];
...
var colorIsDark = function colorIsDark(color) {
  var _getRGBArray = getRGBArray(color),
      ...
      alpha = _getRGBArray2[3]; // if there is an alpha and it's greater than 50%, we can't really tell
...
var getRGBA = function getRGBA(color, opacity) {
  if (color && canExtractRGBArray(color)) {
    var _getRGBArray3 = getRGBArray(color),
        _getRGBArray4 = _slicedToArray(_getRGBArray3, 4),
        red = _getRGBArray4[0],
        green = _getRGBArray4[1],
        blue = _getRGBArray4[2],
        alpha = _getRGBArray4[3];
    ...
    return "rgba(".concat(red, ", ").concat(green, ", ").concat(blue, ", ").concat(normalizedAlpha, ")");
  }
